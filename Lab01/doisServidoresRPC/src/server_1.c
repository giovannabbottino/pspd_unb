...
int main(int argc, char *argv[]){
	...
	if ( server == 0){ 
		...
		*/
		perror("[SERVER 1] Criação do socket falhou");
		exit(0);
	}
	printf("[SERVER 1] Criacao do socket TCP\n");
	...
	if (bind(server, (struct sockaddr *)&server_address, sizeof(server_address))<0){
		perror("[SERVER 1] Não pode fazer bind na porta PORT");
		exit(0);
	}
	printf("[SERVER 1] Conecta na porta\n");

	/*  numero maximo de conexões */
	listen(server, 5);
	printf("[SERVER 1] Escutando\n");

	/* Aceita conexoes direta entre o servidor e cliente */
	client_size = sizeof(client_address);
	client = accept(server, (struct sockaddr *) &client_address, &client_size);
	if ( client == -1 ){
		perror("[SERVER 1] Não pode dar accept na PORT");
		exit(0);
	} 
	printf("[SERVER 1] Aceita conexoes direta\n");

	printf("[SERVER 1] Esperando por dados no IP: %s, porta TCP numero: %d\n", HOST, PORT1);
	int retorno_msg;
	/* Receber a mensagem */
	while(1){
		...
		if(retorno_msg == 0 || errno == 104){
			printf("[SERVER 1] Conexão Encerrada!\n");
			close(client);
			exit(0);
		}else if (retorno_msg < 0){
			perror("[SERVER 1] Não foi possivel receber a mensagem");
		} else{
			printf("[SERVER 1] Recebida a mensagem\n");
			...
			if (send(client, send_message, sizeof(send_message) + 1, 0) < 0){
				perror("[SERVER 1] Não foi possivel responder a mensagem");
			}
		}
	}
}
